...
tm_values = unique(c(round(max(offset_vector)) + 1, round(max(offset_vector)) + seq(from = step_size_tm, to = max_tm, by = step_size_tm)))
...
for(i in 1:length(td_values)){
  for(j in 1:length(tm_values)){
    ...
    res_matrix_ACov_fit.EP[i,j] <- res_fit$RSS
    res_amplitude.EP[((i+j)/2),] <- res_fit$A
    res_C.EP[((i+j)/2),] <- res_fit$C
  }
}
...
EP_diff_values_all_data <- rbind(res_melt_ACov_fit_LL.EP) %>%
  mutate(total_start = Var2 + Var1/2, total_end = Var2 - Var1/2) %>%
  mutate(Inside_Arecheaological_start = ifelse(total_start <= Archeaology_first_humans,"inside","outside" ),
         Inside_Arecheaological_end = ifelse(total_end >= Archeaology_last_neandertals,"inside","outside" )) 



write.csv(x = EP_diff_values_all_data,file = as.character(output_EP),quote = F,row.names = F)
