...
sample_simple_pulse_ACov_sim <- function(min_dist,max_dist,binwidth,A,tm,c=0,noise=0.001,offset,trunc_,round_dec=0.01){
  dist= seq(min_dist,max_dist,binwidth)
  wcov = A*exp(-(dist)*(tm-offset))+c
...
sample_simple_pulse_Seg_sim <- function(n_seg,tm,offset,noise){
  data.x = rexp(n_seg,rate = (tm - offset))
...
tm = as.numeric(snakemake@wildcards$tm)
...
for(i in 1:length(offset)){
  sample.list.SP[[i]] <- sample_simple_pulse_ACov_sim(min_dist=min_dist,max_dist=max_dist,binwidth=binwidth
                                                      ,A=true_A1[i],tm = tm,c=true_C[i],noise = noise,offset  = offset[i],trunc_  = trunc_)
}


### simulated Segment data ###

sample.list.SP_seg_l <- list()
for(i in 1:length(offset)){
  x <- sample_simple_pulse_Seg_sim(n_seg = n_seg,tm = tm,offset = offset[i],noise = noise) 
  x <- x[x >= min_len/100]
  sample.list.SP_seg_l[[i]] <- data.frame(seg=x,ts=rep(offset[i],length(x )))
}


## SP fit
## ACov
sample.list.SP_trunc <- list()
for(i in 1:length(sample.list.SP)){
  sample.list.SP_trunc[[i]] <- sample.list.SP[[i]] %>% filter(dist >= min_cut_ACov)
}



data.in = list(
  dist=lapply(sample.list.SP_trunc, `[[`, 'dist'),
  ACov_res_list=sample.list.SP_trunc,
  y=unlist(lapply(sample.list.SP_trunc, `[[`, 'wcov')),
  weight = unlist(lapply(sample.list.SP_trunc, `[[`, 'weight')),
  offset = offset,
...
Sp_ACov_res <- data.frame(tm = tm_CI_SP_ACov[1],tm_lower = tm_CI_SP_ACov[2],tm_upper = tm_CI_SP_ACov[3],LL=LL_SP_ACov_fit,
                          method="ACov_fit",model="SimplePulse",simulation="SimplePulse",min_l = min_cut_ACov,truncation = trunc_,weighted=weighted,
                          true_tm1= tm,true_tm2= tm,true_td = 1,n_rep = n_rep,noise=noise)

## segments

sample.list.SP_seg_l_trunc <- list()
for(i in 1:length(sample.list.SP_seg_l)){
  sample.list.SP_seg_l_trunc[[i]] <- sample.list.SP_seg_l[[i]] %>% filter(seg >= min_len_Seg/100)
}

l = unlist(lapply(sample.list.SP_seg_l_trunc, `[[`, 'seg'))*100
offset_vector_segment = unlist(lapply(sample.list.SP_seg_l_trunc, `[[`, 'ts'))
...
Sp_seg_res <- data.frame(tm = tm_SP_seg_fit[1],tm_lower = tm_SP_seg_fit[2],tm_upper = tm_SP_seg_fit[3],LL=LL_SP_seg_fit,
                         method="Segments",model="SimplePulse",simulation="SimplePulse",min_l = min_len,truncation = NA,weighted=NA,
                         true_tm1= tm,true_tm2= tm,true_td = 1,n_rep = n_rep,noise=noise)
...
res_matrix_ACov_fit_LL$simulation <- "SimplePulse"
...
res_matrix_Segfit$simulation <- "SimplePulse"
