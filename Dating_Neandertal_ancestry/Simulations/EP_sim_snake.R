...
sample_extended_pulse_ACov_sim <- function(min_dist,max_dist,binwidth,A,td,tm,c=0,noise=0.001,offset,trunc_,round_dec=0.01){
  k = ((4*(tm-offset))/td)^2
  dist= seq(min_dist,max_dist,binwidth)
  wcov = A * ( (1 + ((tm-offset)/k) * dist)^(-k) ) + c
...
sample_extended_pulse_Seg_sim <- function(n_seg,tm,td,offset,noise){
  tm = tm - offset
  k = 1/((td/(4*(tm)))^2)
  data.x = VGAM::rlomax(n = n_seg,scale=((k)/(tm)), shape=((k)+1))
...
tm = as.numeric(strsplit(snakemake@wildcards$tm_td_comb,split = "_")[[1]][1])
td = as.numeric(strsplit(snakemake@wildcards$tm_td_comb,split = "_")[[1]][2])
...
for(i in 1:length(offset)){
  sample.list.EP[[i]] <- sample_extended_pulse_ACov_sim(min_dist=min_dist,max_dist=max_dist,binwidth=binwidth
                                                        ,A=true_A1[i],td = td,tm = tm,c=true_C[i],noise = noise,offset  = offset[i],
                                                        trunc_  = trunc_)
}
...
sample.list.EP_seg_l <- list()
for(i in 1:length(offset)){
  x <- sample_extended_pulse_Seg_sim(n_seg = n_seg,tm = tm,td = td,offset = offset[i],noise=noise) 
  x <- x[x >= min_len/100]
  sample.list.EP_seg_l[[i]] <- data.frame(seg=x,ts=rep(offset[i],length(x )))
}



## ACov
sample.list.EP_trunc <- list()
for(i in 1:length(sample.list.EP)){
  sample.list.EP_trunc[[i]] <- sample.list.EP[[i]] %>% filter(dist >= min_cut_ACov)
}



data.in = list(
  dist=lapply(sample.list.EP_trunc, `[[`, 'dist'),
  ACov_res_list=sample.list.EP_trunc,
  y=unlist(lapply(sample.list.EP_trunc, `[[`, 'wcov')),
  weight = unlist(lapply(sample.list.EP_trunc, `[[`, 'weight')),
  offset = offset,
...
Sp_ACov_res <- data.frame(tm = tm_CI_SP_ACov[1],tm_lower = tm_CI_SP_ACov[2],tm_upper = tm_CI_SP_ACov[3],LL=LL_SP_ACov_fit,
                          method="ACov_fit",model="SimplePulse",simulation="ExtendedPulse",min_l = min_cut_ACov,truncation = trunc_,weighted=weighted,
                          true_tm1= tm,true_tm2= tm,true_td = td,n_rep = n_rep,noise=noise)

## segments

sample.list.EP_seg_l_trunc <- list()
for(i in 1:length(sample.list.EP_seg_l)){
  sample.list.EP_seg_l_trunc[[i]] <- sample.list.EP_seg_l[[i]] %>% filter(seg >= min_len_Seg/100)
}

l = unlist(lapply(sample.list.EP_seg_l_trunc, `[[`, 'seg'))*100
offset_vector_segment = unlist(lapply(sample.list.EP_seg_l_trunc, `[[`, 'ts'))
...
Sp_seg_res <- data.frame(tm = tm_SP_seg_fit[1],tm_lower = tm_SP_seg_fit[2],tm_upper = tm_SP_seg_fit[3],LL=LL_SP_seg_fit,
                         method="Segments",model="SimplePulse",simulation="ExtendedPulse",min_l = min_len,truncation = NA,weighted=NA,
                         true_tm1= tm,true_tm2= tm,true_td = td,n_rep = n_rep,noise=noise)
...
res_matrix_ACov_fit_LL$simulation <- "ExtendedPulse"
res_matrix_ACov_fit_LL$est_tm <- est_tm_EP_ACov_fit_EP
res_matrix_ACov_fit_LL$est_td <- est_td_EP_ACov_fit_EP

res_matrix_ACov_fit_LL$true_tm1 <-  tm
res_matrix_ACov_fit_LL$true_tm2 <-  tm
res_matrix_ACov_fit_LL$true_td <-  td
...
res_matrix_Segfit$true_tm1 <-  tm
res_matrix_Segfit$true_tm2 <-  tm
res_matrix_Segfit$true_td <-  td
res_matrix_Segfit$n_rep <- n_rep

res_matrix_Segfit$min_l <- min_len
res_matrix_Segfit$simulation <- "ExtendedPulse"
