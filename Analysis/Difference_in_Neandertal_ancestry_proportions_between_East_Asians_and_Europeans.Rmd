...
for(i in 1:length(est_method)){
  ...
  Diff_ancient <- data.frame(diff = replicate(n= n_rep,mean(sample(EAS_vs_EURO_random$est[EAS_vs_EURO_random$cluster_ordered == "AncientEAS"],size = 2,replace = T))) - replicate(n= n_rep,mean(sample(EAS_vs_EURO_random$est[EAS_vs_EURO_random$cluster_ordered == "preLGM\nWEurHG"],size = 2,replace = T))),
                             pop = "AncientEAS vs preLGM-WEurHG",method = est_method[i],AncientEASsample = "other")
  
  Diff_present_SGDP <- data.frame(diff = replicate(n= n_rep,mean(sample(EAS_vs_EURO_random$est[EAS_vs_EURO_random$cluster_ordered == "EAS (SGDP)"],size = 2,replace = T))) - replicate(n= n_rep,mean(sample(EAS_vs_EURO_random$est[EAS_vs_EURO_random$cluster_ordered == "WEur (SGDP)"],size = 2,replace = T))),
                                  pop = "EAS vs WEur (SGDP)",method = est_method[i],AncientEASsample = "other" )
  
  Diff_present_1kG <- data.frame(diff = replicate(n= n_rep,mean(sample(EAS_vs_EURO_random$est[EAS_vs_EURO_random$cluster_ordered == "EAS (1kG)"],size = 2,replace = T))) - replicate(n= n_rep,mean(sample(EAS_vs_EURO_random$est[EAS_vs_EURO_random$cluster_ordered == "WEur (1kG)"],size = 2,replace = T))),
                                 pop = "EAS vs WEur (1kG)",method = est_method[i],AncientEASsample = "other")
...
P_random_mean_present_vs_ancient_EUR_EAS <- ggpubr::ggarrange(P_random_pairwise_diff_present_vs_ancient_EUR_EAS,
                                                              P_random_mean_diff_present_vs_ancient_EUR_EAS,ncol = 2)

ggsave(paste0(folder_path_save,"P_random_diff_present_vs_ancient_EUR_EAS.png"),P_random_mean_present_vs_ancient_EUR_EAS,device = "png",width = 12,height = 5)
