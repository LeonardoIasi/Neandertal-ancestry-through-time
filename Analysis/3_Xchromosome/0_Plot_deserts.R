...
regions = read.table('Deserts.txt', col.names = c('CHROM','START', 'END')) %>%
	filter(CHROM == 'X')
...
rate_diffs %>%
	write.table('Rec_map_X.txt', row.names = F, quote = F, sep = '\t')
...
ggsave('2_Entire_X_deserts.pdf', width = 12, height = 18)
