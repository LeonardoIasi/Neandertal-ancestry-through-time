...
# folder
folder_path = "~/EMH_Introgression_Project/Introgression_Detection/Paper_Figures/Supplement/"

'%notin%' <- Negate('%in%')

PAM_check_k<- function(F3_matrix,max_k,gap_stat=F){
  
  x1 <- fviz_nbclust(F3_matrix, pam, method = "silhouette",k.max = max_k,nboot = 100)
  x2 <- fviz_nbclust(F3_matrix, pam, method = "wss",k.max = max_k,nboot = 100)
  if(gap_stat == T){
    x3 <- fviz_nbclust(F3_matrix, pam, method = "gap_stat",k.max = max_k,nboot = 100)
...
kmeans_check_k<- function(F3_matrix,max_k,gap_stat=F){
  
  x1 <- fviz_nbclust(F3_matrix, kmeans, method = "silhouette",k.max = max_k,nboot = 100)
  x2 <- fviz_nbclust(F3_matrix, kmeans, method = "wss",k.max = max_k,nboot = 100)
  if(gap_stat == T){
    x3 <- fviz_nbclust(F3_matrix, kmeans, method = "gap_stat",k.max = max_k,nboot = 100)
    return(list(x1,x2,x3))    
  }
  else{
    return(list(x1,x2))  
...
MDS_with_PAM_clusters <- function(F3_matrix,F3_matrix_dis,k,only_cluster=F){
  F3_MDS_frags <- as.data.frame(cmdscale(
    as.dist(F3_matrix_dis),
    k=10,eig = T)$points)
  
  pam_fit <- cluster::pam(
    dist(F3_matrix,method = "euclidean"),
    #F3_matrix,
    diss = TRUE, k = k)
  
  F3_MDS_frags$cluster <- pam_fit$clustering
  
  F3_MDS_frags_anno <- F3_MDS_frags %>% mutate(sample_name=colnames(F3_matrix)) %>%
    inner_join(.,Joint_Meta,by=c("sample_name"="F3_pop_annotation")) %>%
    filter(duplicated(sample_name) == FALSE)
  
  MDS_clustered <- ggplot(data=F3_MDS_frags_anno, aes(x=V1, y=V2,color=as.factor(cluster),label= sample_name)) +
    geom_point() +
    xlab("K1") +
    ylab("K2") +
  ...
  if(only_cluster == T){
    return(F3_MDS_frags_anno[,c("sample_name","cluster")])
  }
  else{
    return(MDS_clustered)
  }
  
}

MDS_with_kmeans_clusters <- function(F3_matrix,F3_matrix_dis,k,only_cluster=F){
  F3_MDS_frags <- as.data.frame(cmdscale(
    as.dist(F3_matrix_dis),
    k=10,eig = T)$points)
  ...
  F3_MDS_frags$cluster <- kmeans_fit$cluster
...
The calculations for all pairwise F3 are done using ADMIXTOOLS2 (in the script home/leonardo_iasi/EMH_Introgression_Project/Introgression_Detection/Analysis/Calculate_F3_on_1240k.R). We only consider sites where at least half of the ancient individuals have SNP information. The F2 from which all the F3's are calculated are done so with parameters:
 * maxmiss = 1
 * auto_only = T
...
Joint_Meta <- read.csv("~/EMH_Introgression_Project/Introgression_Detection/Joint_Meta_data.csv",sep = ",") %>% mutate(F3_pop_annotation = ifelse(ML_BP_Mean == 0 ,pop,sample_name)) 
Joint_Meta$pop[Joint_Meta$pop == "Sahrawi"] = "Saharawi"
Joint_Meta$F3_pop_annotation[Joint_Meta$F3_pop_annotation == "Sahrawi"] = "Saharawi"
Joint_Meta = Joint_Meta %>%  mutate(ID = c(Joint_Meta$pop[Joint_Meta$ML_BP_Mean == 0],Joint_Meta$sample_name[Joint_Meta$ML_BP_Mean > 0]))

# All together

res_f3_gtLH_Autosomes <- read.csv("~/EMH_Introgression_Project/Introgression_Detection/Analysis/F3_stats/admixfrog0.7/Autosomes/res_f3_AF_gtLH_A1240k_Shared_Map.csv")
...
write.csv(F3_MDS_frags_anno,"~/EMH_Introgression_Project/Introgression_Detection/Analysis/Genetic_clustering_F3_MDS_anno.csv",quote = F,row.names = F)
...
write.csv(Joint_Meta,"~/EMH_Introgression_Project/Introgression_Detection/Joint_Meta_data_genetic_clusters_using_1240k.csv",quote = F,row.names = F)

```

## Write output files

```{r eval=F,echo=F, result='hide'}
write.csv(Joint_Meta,"~/EMH_Introgression_Project/Introgression_Detection/Joint_Meta_data_genetic_clustersF3d1240k.csv",quote = F,row.names = F)

Joint_Meta2 <- read.csv("~/EMH_Introgression_Project/Introgression_Detection/Joint_Meta_data_genetic_clustersED.csv") %>% mutate(PCA_pops = ifelse(ML_BP_Mean == 0 ,pop,sample_name))
Joint_Meta2 <- Joint_Meta2 %>% dplyr::distinct()
Joint_Meta2 <- inner_join(Joint_Meta2,Joint_Meta[,c("sample_name","SuperclusterF3D","clusterF3D")],by="sample_name")

write.csv(Joint_Meta2,"~/EMH_Introgression_Project/Introgression_Detection/Joint_Meta_data_genetic_clusters_using_1240k.csv",quote = F,row.names = F)
