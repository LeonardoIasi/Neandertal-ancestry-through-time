...
```{r eval=T,echo=F,results='hide'}
# folder
folder_path = "~/EMH_Introgression_Project/Introgression_Detection/Preliminary_figures_Updates_admixfrog0.7/"

folder_path_data = "/home/leonardo_iasi/EMH_Introgression_Project/Introgression_Detection/Preliminary_Paper_Figures_Data/"
...
Joint_Meta <- read.csv("~/EMH_Introgression_Project/Introgression_Detection/Joint_Meta_data_genetic_clusters_using_1240k.csv") %>% 
  filter(sample_name != "Mota",SuperclusterF3D != "Africa") %>% mutate(sample_name=gsub( "-", ".", sample_name))
...
dir_path_EMH_all <- list.files(path=paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_EMH_admixfrog0.7_Shared_Map/rle/error2/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/"),pattern = paste0("archaicadmixtureAPX.rle",penalty,".xz.anno_all_types$"),full.names = T)


dir_path_SGDP_all <- list.files(path=paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_SGDP_admixfrog0.7_Shared_Map/rle/gtmode/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/"),pattern = paste0("archaicadmixtureAPX.rle",penalty,".xz.anno_all_types$"),full.names = T)


EMH_file_names_NEA_state = write_bed_from_rle(directory_path = dir_path_EMH_all,split_char = "_",type_ = called_type,target_ = states,chrom_ = chrom_,path = paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_EMH_admixfrog0.7_Shared_Map/rle/error2/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/"),min_len = min_len_ancient,min_bp_len = min_bp_len_ancient,min_SNP = min_SNP,optional_col = "frag_ID")

SGDP_file_names_NEA_state = write_bed_from_rle(directory_path = dir_path_SGDP_all,split_char = "_archaic",type_ = called_type,target_ = states,chrom_ = chrom_,path = paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_SGDP_admixfrog0.7_Shared_Map/rle/gtmode/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/"),min_len = min_len_present,min_bp_len = min_bp_len_present,min_SNP = min_SNP,optional_col = "frag_ID")


system(paste("bedtools multiinter -i ",paste(c(EMH_file_names_NEA_state,SGDP_file_names_NEA_state), collapse=' ',sep = " ")," > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_unmasked_matrix.bed",sep=" "))

system("bedtools subtract -a ~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_unmasked_matrix.bed  -b ~/EMH_Introgression_Project/Introgression_Detection/ref/bed/AA_Mask_joint.bed > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_masked_matrix.bed
")
...
write.table(x = sample_names,file = "~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_unmasked_matrix_sample_names.txt",quote = F,row.names = F,col.names = F)

Nea_ancestry_ind_count_matrix_unmasked <- read.table("~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_unmasked_matrix.bed",col.names = c("chrom","start","end","n_ind","ind_index",sample_names))

write.table(Nea_ancestry_ind_count_matrix_unmasked,"~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_unmasked_matrix.csv",quote = F,row.names = F)

Nea_ancestry_ind_count_matrix <- read.table("~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_masked_matrix.bed",col.names = c("chrom","start","end","n_ind","ind_index",sample_names))

write.table(Nea_ancestry_ind_count_matrix,"~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_masked_matrix.csv",quote = F,row.names = F)
...
```{r eval=T,echo=F,results='hide'}
Segment_cluster_data <- read.table("~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/All_NEA_state_with_indcount_masked_matrix.csv",sep=" ", header=T)

samples_x <- Joint_Meta  %>% filter(SuperclusterF3D != "Africa") %>% .$sample_name
...
#save(Segment_PCA_all_anno,Segment_PCA_all_importance,file = "~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/PCA_on_NEA_ancestry.RData")
...
for( pop in unique(Joint_Meta[[cluster_used]][Joint_Meta$SuperclusterF3D != "Africa"])){
...
#save(Segment_PCA_all_loadings_red_long_PC1,file = "~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/PCA_on_NEA_ancestry_Highest_Loadings_PC1.RData")
#save(Segment_PCA_all_loadings_red_long_PC2,file = "~/EMH_Introgression_Project/Introgression_Detection/Analysis/PCA_Neandertal_Ancestry/PCA_on_NEA_ancestry_Highest_Loadings_PC2.RData")
...
dir_path_EMH <- list.files(path=paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_EMH_admixfrog0.7_Shared_Map/rle/error2/length_bin_15/5000/AFR-NEA-DEN/",genetic_map),pattern = paste0("archaicadmixtureAPX.rle",penalty,".xz.anno_all_types$"),full.names = T)


dir_path_SGDP <- list.files(path=paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_SGDP_admixfrog0.7_Shared_Map/rle/gtmode/length_bin_15/5000/AFR-NEA-DEN/",genetic_map),paste0("archaicadmixtureAPX.rle",penalty,".xz.anno_all_types$"),full.names = T)



EMH_rle_all <- get_rle_files(dir_path_EMH,"_",type_ = called_type,target_ = states,chrom_,sep_char = ",") %>% 
  drop_na() %>% filter(map_len >= min_len_ancient, pos_len >= min_bp_len_ancient)
SGDP_rle_all <- get_rle_files(dir_path_SGDP,"_archaic",type_ = called_type,target_ = states,chrom_,sep_char=",") %>% filter(map_len >= min_len_present, pos_len >= min_bp_len_present) %>%
...
for(pop_ in unique(Joint_Meta_unrelated[[cluster_used]])){
  ...
  NEA_Overlap_data = All_rle %>% filter(sample %notin% c("AfanasievoSon1","AfanasievoSon2")) %>%
    inner_join(.,Joint_Meta_unrelated[,c("sample_name","SuperclusterF3D",cluster_used,"pop","ML_BP_Mean")],by=c("sample"="sample_name")) %>%
  ...
  write.table(data.frame(chrom=NEA_Overlap_data$chrom,start=NEA_Overlap_data$start_pos,end=NEA_Overlap_data$end_pos,name=NEA_Overlap_data$group_id),file =paste("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/",pop_,"_NEA_ancestry_unmasked.bed",sep=""),quote = F,row.names = F,col.names = F,sep = '\t')
  Pop_bed_names = c(Pop_bed_names,paste("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/",pop_,"_NEA_ancestry_unmasked.bed",sep=""))
  
}

system(paste("bedtools multiinter -i ",paste(Pop_bed_names, collapse=' ',sep = " ")," > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Pop_NEA_ancestry_unmasked.bed",sep=" "))

system("bedtools subtract -a ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Pop_NEA_ancestry_unmasked.bed -b ~/EMH_Introgression_Project/Introgression_Detection/ref/bed/AA_Mask_joint.bed  > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Pop_NEA_ancestry_masked.bed
")
```
...
for(pop_ in unique(Joint_Meta_unrelated[[cluster_used]])){
  ...
  write.table(data.frame(chrom=NEA_Overlap_data$chrom,start=NEA_Overlap_data$start_pos,end=NEA_Overlap_data$end_pos,name=NEA_Overlap_data$id),file =paste("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Single_sample_from_",pop_,"_NEA_ancestry_unmasked.bed",sep=""),quote = F,row.names = F,col.names = F,sep = '\t')
  Pop_bed_names = c(Pop_bed_names,paste("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Single_sample_from_",pop_,"_NEA_ancestry_unmasked.bed",sep=""))
  
}

system(paste("bedtools multiinter -i ",paste(Pop_bed_names, collapse=' ',sep = " ")," > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Single_sample_from_Pop_NEA_ancestry_unmasked.bed",sep=" "))

system("bedtools subtract -a ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Single_sample_from_Pop_NEA_ancestry_unmasked.bed -b ~/EMH_Introgression_Project/Introgression_Detection/ref/bed/AA_Mask_joint.bed  > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Single_sample_from_Pop_NEA_ancestry_masked.bed
")

Single_Sample_Pop_masked_multiinter_all_combinations <- read.table("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Single_sample_from_Pop_NEA_ancestry_masked.bed",col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_unrelated[[cluster_used]]))) %>%
...
Single_Sample_Pop_masked_multiinter <- read.table("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Single_sample_from_Pop_NEA_ancestry_masked.bed",col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_unrelated[[cluster_used]]))) %>%
...
for(i in 1:n_bootstrap){
  ...
  for(pop_ in unique(Joint_Meta_unrelated[[cluster_used]])){
    ...
    write.table(data.frame(chrom=NEA_Overlap_data$chrom,start=NEA_Overlap_data$start_pos,end=NEA_Overlap_data$end_pos,name=NEA_Overlap_data$id),file =paste("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_",pop_,"_NEA_ancestry_unmasked_bootstrap",i,".bed",sep=""),quote = F,row.names = F,col.names = F,sep = '\t')
    Pop_bed_names = c(Pop_bed_names,paste("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_",pop_,"_NEA_ancestry_unmasked_bootstrap",i,".bed",sep=""))
    
  }

  system(paste("bedtools multiinter -i ",paste(Pop_bed_names, collapse=' ',sep = " ")," > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_unmasked_bootstrap",i,".bed",sep=""))
  
  system(paste0("bedtools subtract -a ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_unmasked_bootstrap",i,".bed -b ~/EMH_Introgression_Project/Introgression_Detection/ref/bed/AA_Mask_joint.bed  > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked_bootstrap",i,".bed
  "))
  system(paste0("rm ",paste(Pop_bed_names, collapse=' ',sep = " ")," ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_unmasked_bootstrap",i,".bed"))
}

#for(i in 1:n_bootstrap){
#  if(i == 1){
#    Single_Sample_Pop_masked_multiinter_all_combinations_bootstrap <- #read.table(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked_bootstra#p",i,".bed"),col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_unrelated[[cluster_used]]))) %>%
#  mutate(combination_names = apply(.[6:length(.)], 1, function(x) paste(names(which(x >0)),collapse = ","))) %>%
#  group_by(combination_names,n_overlaps) %>% 
#  mutate(seg_length = end - start) %>% summarize(Unique_segs_in_Mb = sum(seg_length)/1e6) %>% mutate(bootstrap = i)
#  }else{
#    x <- read.table(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked#_bootstrap",i,".bed"),col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_unrelated[[cluster_used]]))) %>%
#  mutate(combination_names = apply(.[6:length(.)], 1, function(x) paste(names(which(x >0)),collapse = ","))) %>%
#  group_by(combination_names,n_overlaps) %>% 
#  mutate(seg_length = end - start) %>% summarize(Unique_segs_in_Mb = sum(seg_length)/1e6)  %>% mutate(bootstrap = i)
#    
...
for(i in 1:n_bootstrap){
  if(i == 1){
    Single_Sample_Pop_masked_multiinter_all_combinations_bootstrap <- read.table(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked_bootstrap",i,".bed"),col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_unrelated[[cluster_used]]))) %>%
  ...
  mutate(Pop = ifelse(Pop == "Siberia.Americas","Siberia&Americas",Pop),Pop = gsub( "\\.", "-", Pop)) %>% mutate(bootstrap = i)
    ...
    x <- read.table(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked_bootstrap",i,".bed"),col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_unrelated[[cluster_used]]))) %>%
  mutate(seg_length = end - start) %>%
  mutate(combination_names = apply(.[6:(length(.)-1)], 1, function(x) paste(names(which(x >0)),collapse = ","))) %>%
  mutate(across(6:(length(.)-2), ~ ifelse(. == 1, cur_column(), 0)))  %>% 
  group_by(n_overlaps,combination_names,!!!syms(colnames(.)[6:(length(.)-2)]))  %>% 
...
#write.table(Single_Sample_Pop_masked_multiinter_all_combinations_bootstrap,"~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_Sample_Pop_masked_multiinter_all_combinations_bootstrap.csv",quote = F,row.names = F,sep = "\t")
...
for(i in 1:n_bootstrap){
  ...
  for(pop_ in unique(Joint_Meta_wo_Oase[[cluster_used]])){
    ...
    NEA_Overlap_data = All_rle %>% filter(sample %notin% c("AfanasievoSon1","AfanasievoSon2")) %>%
      inner_join(.,Joint_Meta_wo_Oase[,c("sample_name","SuperclusterF3D",cluster_used,"pop","ML_BP_Mean")],by=c("sample"="sample_name")) %>%
      filter(sample == pop_ind) %>% mutate(start_pos = pos,end_pos = pos_end) %>% 
    mutate(pos_len = end_pos - start_pos) %>% mutate(chrom = ifelse(chrom == "X","23",chrom)) %>% 
    mutate_at(c('chrom'), as.numeric) %>% arrange(.,chrom,start_pos,end_pos)
    ...
    write.table(data.frame(chrom=NEA_Overlap_data$chrom,start=NEA_Overlap_data$start_pos,end=NEA_Overlap_data$end_pos,name=NEA_Overlap_data$id),file =paste("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_",pop_,"_NEA_ancestry_unmasked_bootstrap_wo_Oase",i,".bed",sep=""),quote = F,row.names = F,col.names = F,sep = '\t')
    Pop_bed_names = c(Pop_bed_names,paste("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_",pop_,"_NEA_ancestry_unmasked_bootstrap_wo_Oase",i,".bed",sep=""))
    
  }

  system(paste("bedtools multiinter -i ",paste(Pop_bed_names, collapse=' ',sep = " ")," > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_unmasked_bootstrap_wo_Oase",i,".bed",sep=""))
  
  system(paste0("bedtools subtract -a ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_unmasked_bootstrap_wo_Oase",i,".bed -b ~/EMH_Introgression_Project/Introgression_Detection/ref/bed/AA_Mask_joint.bed  > ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked_bootstrap_wo_Oase",i,".bed
  "))
  system(paste0("rm ",paste(Pop_bed_names, collapse=' ',sep = " ")," ~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_unmasked_bootstrap_wo_Oase",i,".bed"))
}

#for(i in 1:n_bootstrap){
#  if(i == 1){
#    Single_Sample_Pop_masked_multiinter_all_combinations_bootstrap <- #read.table(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked_bootstra#p_wo_Oase",i,".bed"),col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_wo_Oase[[cluster_used]]))) %>%
#  mutate(combination_names = apply(.[6:length(.)], 1, function(x) paste(names(which(x >0)),collapse = ","))) %>%
#  group_by(combination_names,n_overlaps) %>% 
#  mutate(seg_length = end - start) %>% summarize(Unique_segs_in_Mb = sum(seg_length)/1e6) %>% mutate(bootstrap = i)
#  }else{
#    x <- read.table(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked#_bootstrap_wo_Oase",i,".bed"),col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_wo_Oase[[cluster_used]]))) %>%
#  mutate(combination_names = apply(.[6:length(.)], 1, function(x) paste(names(which(x >0)),collapse = ","))) %>%
#  group_by(combination_names,n_overlaps) %>% 
#  mutate(seg_length = end - start) %>% summarize(Unique_segs_in_Mb = sum(seg_length)/1e6)  %>% mutate(bootstrap = i)
#    
...
for(i in 1:n_bootstrap){
  if(i == 1){
    Single_Sample_Pop_masked_multiinter_all_combinations_bootstrap_wo_Oase <- read.table(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked_bootstrap_wo_Oase",i,".bed"),col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_wo_Oase[[cluster_used]]))) %>%
  mutate(seg_length = end - start) %>%
  mutate(combination_names = apply(.[6:(length(.)-1)], 1, function(x) paste(names(which(x >0)),collapse = ","))) %>%
  mutate(across(6:(length(.)-2), ~ ifelse(. == 1, cur_column(), 0)))  %>% 
  group_by(n_overlaps,combination_names,!!!syms(colnames(.)[6:(length(.)-2)]))  %>% 
  ...
  mutate(Pop = ifelse(Pop == "Siberia.Americas","Siberia&Americas",Pop),Pop = gsub( "\\.", "-", Pop)) %>% mutate(bootstrap = i)
    ...
    x <- read.table(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_sample_from_Pop_NEA_ancestry_masked_bootstrap_wo_Oase",i,".bed"),col.names = c("chrom","start","end","n_overlaps","combination",unique(Joint_Meta_wo_Oase[[cluster_used]]))) %>%
  mutate(seg_length = end - start) %>%
  mutate(combination_names = apply(.[6:(length(.)-1)], 1, function(x) paste(names(which(x >0)),collapse = ","))) %>%
  mutate(across(6:(length(.)-2), ~ ifelse(. == 1, cur_column(), 0)))  %>% 
  group_by(n_overlaps,combination_names,!!!syms(colnames(.)[6:(length(.)-2)]))  %>% 
...
#write.table(Single_Sample_Pop_masked_multiinter_all_combinations_bootstrap,"~/EMH_Introgression_Project/Introgression_Detection/Analysis/Unique_NEA_Segments/Unique_Segments_bootstrap/Single_Sample_Pop_masked_multiinter_all_combinations_bootstrap_wo_Oase.csv",quote = F,row.names = F,sep = "\t")
...
## Segment Correlation vs. 

### Data

```{r eval=T,echo=F,results='hide'}
SGDP_NEA_bins_raw = read.csv(paste("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_SGDP_admixfrog0.7_Shared_Map/stats/gtmode/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/All_bins_NEA_PresentDaySGDP_archaicadmixtureAPX.bin_called",penalty,"_min_len",min_len_present,"_min_len_pos",min_bp_len_present,"_min_n_all_SNP",min_SNP,"_min_n_SNP",min_all_SNP,".xz",sep=""))

sample_list_present = Joint_Meta %>% filter(time == "present" ) %>% 
  filter(SuperclusterF3D != "Africa") %>% dplyr::select(pop) %>% distinct()
...
EMH_NEA_bins_raw = read.csv(paste("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_EMH_admixfrog0.7_Shared_Map/stats/error2/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/All_bins_NEA_CurratedEMHPublished_archaicadmixtureAPX.bin_called",penalty,"_min_len",min_len_ancient,"_min_len_pos",min_bp_len_ancient,"_min_n_all_SNP",min_SNP,"_min_n_SNP",min_all_SNP,".xz",sep=""))  %>% dplyr::select(chrom,map,!!sample_list_ancient)
...
non_callable_bins <- read.csv(paste0("/home/leonardo_iasi/EMH_Introgression_Project/Introgression_Detection/admixfrogREF/ref/published/EMHPanel/Shared_Map/bin_ref/",genetic_map,"/archaicadmixtureAPX_admixfrog_bins_ref_non_callable_bins.csv"),col.names = c("chrom","map","pos","id"))
...
present_day_inds = Joint_Meta %>% filter(ML_BP_Mean == 0,SuperclusterF3D != "Africa") %>% distinct(ID, .keep_all = TRUE)
...
ancient_inds_same_site = Joint_Meta %>% filter(ML_BP_Mean > 0,SuperclusterF3D != "Africa") %>%
  mutate(Longitude = round(Longitude,2),Latitude = round(Latitude,2)) %>% 
  group_by(Longitude,Latitude) %>%
  filter(n()>1)

ancient_inds_list = Joint_Meta %>% filter(ML_BP_Mean > 0,SuperclusterF3D != "Africa") %>%
  mutate(Longitude = round(Longitude,2),Latitude = round(Latitude,2)) %>% 
  group_by(Longitude,Latitude) %>%
...
The F3 values are precomputed for every pairwise population using Mbuti as an outgroup. The populations for present day individuals are the SGDP assigned populations and for ancient individuals it is done per individual. For all F3 only SNPs are used where at least 50 % of the ancients have data. The F3 are either computed on the archaicadmixtureAPX set or 1240k and for autosomes and X chrom separately. The estimates on the X chrom are only done using female individuals!

```{r echo=TRUE, eval=TRUE}
# archaicadmixtureAPX
res_f3_gtLH_archaicadmixtureAPX_Autosomes <- read.csv(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/F3_stats/admixfrog0.7/Autosomes/res_f3_AF_gtLH_A1240k_",genetic_map,".csv"))
...
res_f3_gtLH_1240k_Autosomes <- read.csv(paste0("~/EMH_Introgression_Project/Introgression_Detection/Analysis/F3_stats/admixfrog0.7/Autosomes/res_f3_AF_gtLH_archaicadmixtureX_",genetic_map,".csv"))
...
#save(df_poporder,melted_combined_anno_red,Joint_values,All_Sample_list,file = "~/EMH_Introgression_Project/Introgression_Detection/Analysis/NEA_cor_vs_F3_plots.RData")
...
dir_path_EMH <- list.files(path=paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_EMH_admixfrog0.7_Shared_Map/Diagnostic_sites/error2/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/",DiagSites,"/"),pattern = "rle0.25.match_summary.xz",full.names = T)


dir_path_SGDP <- list.files(path=paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/New_REF_AA_with_APX_SGDP_admixfrog0.7_Shared_Map/Diagnostic_sites/gtmode/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/",DiagSites,"/"),pattern = "rle0.25.match_summary.xz",full.names = T)


EMH_rle_all <- get_rle_files(directory_path = dir_path_EMH,split_char = "_",type_ = called_type,target_ = states,chrom_ = chrom_)  %>% 
   filter(map_len >= min_len_ancient, pos_len >= min_bp_len_ancient)


SGDP_rle_all <- get_rle_files(dir_path_SGDP,"_archaic",type_ = called_type,target_ = states,chrom_) %>% filter(map_len >= min_len_present, pos_len >= min_bp_len_present) %>%
  mutate(sample = gsub("-", "\\.", sample))

All_rle_Matching_ascertainment <- rbind(SGDP_rle_all,EMH_rle_all) %>% mutate(sites = "all")


MatchingRates_file_MatchingAScerteinment_anno <- inner_join(All_rle_Matching_ascertainment,Joint_Meta[,c("sample_name","SuperclusterF3D",cluster_used,"Cov","ML_BP_Mean","pop","Sex","x_chrom_captured","DataType","ave_cont")],by=c("sample"="sample_name")) %>% 
...
Bootsrtap_of_Means_MatchingAScerteinment_non_overlapping  <- list()
n_bootstraps <- 1000
for(i in 1:n_bootstraps){
  Bootsrtap_of_Means_MatchingAScerteinment_non_overlapping[[i]] <- MatchingRates_file_MatchingAScerteinment_anno_non_overlapping %>% 
    filter(chrom != "X", n_all_snps > 10) %>% 
    group_by(!!sym(cluster_used),DataType) %>%
    dplyr::summarize(Mean_match = mean(sample(BT_prop_pHcount_matching_Chagyrskaya_mean,size = length(BT_prop_pHcount_matching_Chagyrskaya_mean),replace = T),na.rm = T)) %>%
    mutate(BT = i)
  
}

Bootsrtap_of_Means_MatchingAScerteinment_non_overlapping <- do.call("rbind",Bootsrtap_of_Means_MatchingAScerteinment_non_overlapping)

save(Bootsrtap_of_Means_MatchingAScerteinment_non_overlapping,file = paste0(folder_path_data,"MatchingRates_file_anno_non_overlapping_",genetic_map,"_",DiagSites,".RData"))

save(Bootsrtap_of_Means_MatchingAScerteinment_non_overlapping,file = paste0(folder_path_data,"Bootsrtap_of_Means_non_overlapping",genetic_map,"_",DiagSites,".RData"))
...
All_rle_OnlyVariable <- rbind(SGDP_rle_all,EMH_rle_all) %>% mutate(sites = "all")


MatchingRates_file_OnlyVariable_anno <- inner_join(All_rle_OnlyVariable,Joint_Meta[,c("sample_name","SuperclusterF3D",cluster_used,"Cov","ML_BP_Mean","pop","Sex","x_chrom_captured","DataType","ave_cont")],by=c("sample"="sample_name")) %>% 
  mutate(time_slice = ifelse(ML_BP_Mean >= 30000,30000,ifelse(ML_BP_Mean >= 15000,15000,ifelse(ML_BP_Mean >= 5000,5000,0)))) %>%
  mutate(time_period = ifelse(ML_BP_Mean >= 15000,"pre-LGM",ifelse(ML_BP_Mean > 0,"post-LGM","Present-Day"))) %>%
...
Bootsrtap_of_Means_OnlyVariable_non_overlapping  <- list()
n_bootstraps <- 1000
for(i in 1:n_bootstraps){
  Bootsrtap_of_Means_OnlyVariable_non_overlapping[[i]] <- MatchingRates_file_OnlyVariable_anno_non_overlapping %>% 
    filter(chrom != "X", n_all_snps > 10) %>% 
    group_by(!!sym(cluster_used),DataType) %>%
    dplyr::summarize(Mean_match = mean(sample(BT_prop_pHcount_matching_Chagyrskaya_mean,size = length(BT_prop_pHcount_matching_Chagyrskaya_mean),replace = T),na.rm = T)) %>%
    mutate(BT = i)
...
Bootsrtap_of_Means_OnlyVariable_non_overlapping <- do.call("rbind",Bootsrtap_of_Means_OnlyVariable_non_overlapping)

save(MatchingRates_file_OnlyVariable_anno_non_overlapping,file = paste0(folder_path_data,"MatchingRates_file_anno_non_overlapping_",genetic_map,"_",DiagSites,".RData"))

save(Bootsrtap_of_Means_OnlyVariable_non_overlapping,file = paste0(folder_path_data,"Bootsrtap_of_Means_non_overlapping",genetic_map,"_",DiagSites,".RData"))
```
...
## all
DiagSites = "all"
K14_shotgun <- get_rle_files(paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/TestLineageAssignment_Shared_Map/Diagnostic_sites/error2/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/",DiagSites,"/Kostenki14Shotgun_archaicadmixtureAPX.rle0.25.match_summary.xz"),"_",c("state"),states,chrom_) %>% 
   filter(map_len >= min_len_ancient, pos_len >= min_bp_len_ancient) %>% mutate(Data = "Shotgun",Cov = 2.52)

K14_capture <- get_rle_files(paste0("/mnt/diversity/leonardo_iasi/EMH_Introgression_Project/TestLineageAssignment_Shared_Map/Diagnostic_sites/error2/length_bin_15/5000/AFR-NEA-DEN/",genetic_map,"/",DiagSites,"/Kostenki14AA_archaicadmixtureAPX.rle0.25.match_summary.xz"),"_",c("state"),states,chrom_) %>% 
...
Not very informative all are closest to Vindija.
...
MatchingRates_file_anno_class_ind <- MatchingRates_file_all_anno  %>% filter(target == "NEA") %>% filter(chrom != "X", n_all_snps > 10) %>% 
  group_by(sample,SuperclusterF3D,(!!sym(cluster_used)),time_period,target,Sites) %>% summarise(
...
Tip_branch_matching_whole_ind_Superpop_non_overlapping <- MatchingRates_file_anno_class_ind %>%
  filter(Sites == "MatchingAscertainment") %>%
  ggplot(.,aes(x = factor(1),fill=factor(Highest_matching_archaic))) + 
    facet_wrap(time_period~SuperclusterF3D,) +
...
```{r echo=FALSE, eval=TRUE, results='hide'}
# SUPERPOPULATIONS
MatchingRates_file_anno_class_table_non_overlapping = MatchingRates_file_anno_class_non_overlapping  %>% filter(Highest_matching_archaic %in% c("Altai", "Chagyrskaya", "Denisova", "Vindija33.19")) %>% dplyr::select(SuperclusterF3D,Highest_matching_archaic)

MatchingRates_file_anno_class_table_non_overlapping = as.data.frame(table(MatchingRates_file_anno_class_table_non_overlapping)) %>% pivot_wider(names_from = Highest_matching_archaic, values_from = Freq) %>% column_to_rownames(.,var = "SuperclusterF3D") %>% dplyr::select(!Denisova)
...
MatchingRates_file_anno_class_poisson_data_table_Supercluster <- MatchingRates_file_anno_class_multnomial_data %>%
  mutate(Highest_matching_archaic = ifelse(Highest_matching_archaic == "Vindija33.19","Vindija",Highest_matching_archaic)) %>%
  group_by(Highest_matching_archaic,SuperclusterF3D,Cov,DataType,ML_BP_Mean,ave_cont,Sites) %>% summarise(count = n()) %>%
  pivot_wider(names_from = Highest_matching_archaic, values_from = count) %>% replace(is.na(.), 0)

dat_Superpop.1 <- list(popul =  as.numeric(as.factor(MatchingRates_file_anno_class_poisson_data_table_Supercluster$SuperclusterF3D)),
...
plotting_GLM_model_fn <- function(model_superpop,data_superpop,model_pop,data_pop,cluster_used){
  ...
  pop_intercepts <- list()
  for(grp in 1:length(unique(data_superpop$SuperclusterF3D))){
    pop_intercepts[[grp]] <-   matrix(sapply(1:length(post$a_1[,1]), function(i) c(
      exp(post$a_1[i,grp])/(exp(post$a_1[i,grp])+exp(post$a_2[i,grp])+exp(post$a_3[i,grp])+exp(post$a_4[i,grp])),
      exp(post$a_2[i,grp])/(exp(post$a_1[i,grp])+exp(post$a_2[i,grp])+exp(post$a_3[i,grp])+exp(post$a_4[i,grp])),
      exp(post$a_3[i,grp])/(exp(post$a_1[i,grp])+exp(post$a_2[i,grp])+exp(post$a_3[i,grp])+exp(post$a_4[i,grp])),
  ...
  pop_int <- c()
  for(grp in 1:length(unique(data_superpop$SuperclusterF3D))){
    pop_int <- rbind(pop_int,data.frame(levels(as.factor(data_superpop$SuperclusterF3D))[grp],pop_intercepts[[grp]]))
  }
  colnames(pop_int) <- c("Pop","ALT","CHA","DEN","VIN")
  ...
  pop_diff <- data.frame((pop_intercepts[[1]] - pop_intercepts[[1]]), pivot = 1,pop = 1)
  for(pvt in 1:length(unique(data_superpop$SuperclusterF3D))){
    for(grp in 1:length(unique(data_superpop$SuperclusterF3D))){
      pop_diff <- rbind(pop_diff,data.frame((pop_intercepts[[grp]] - pop_intercepts[[pvt]]), pivot = pvt,pop = grp))
    }
  }
  ...
  for(i in unique(pop_diff$PivotPop)){
    pop_diff$PivotPop[pop_diff$PivotPop == i] <- levels(as.factor(data_superpop$SuperclusterF3D))[i]
    pop_diff$Pop[pop_diff$Pop == i] <- levels(as.factor(data_superpop$SuperclusterF3D))[i]
...
Superpop_list <- c()
for(grp in 1:length(unique(MatchingRates_file_anno_class_poisson_data_table_Supercluster$SuperclusterF3D))){
  for(arch in c("ALT","CHA","DEN","VIN")){
    Superpop_list <- c(Superpop_list,paste0(levels(as.factor(MatchingRates_file_anno_class_poisson_data_table_Supercluster$SuperclusterF3D))[grp],"_raw_prop_",arch))
  }
}
...
# EM algorithm for fitting a mixture of two Gaussian distributions
EM_algorithm <- function(data, initial_params, max_iter = 1000, tol = 1e-8,constrain = F) {
  ...
  for (iter in 1:max_iter) {
    ...
    if(constrain){
      #mu[1] <- ifelse(mu[1] + 3*sigma[1] < 1 && mu[1] - 3*sigma[1] > 0,mu[1],prev_mu[1])
      #sigma[1] <- ifelse(mu[1] + 3*sigma[1] < 1 && mu[1] - 3*sigma[1] > 0,sigma[1],prev_sigma[1])
      #mu[2] <- ifelse(mu[2] + 3*sigma[2] < 1 && mu[2] - 3*sigma[2] > 0,mu[2],prev_mu[2])
      #sigma[2] <- ifelse(mu[2] + 3*sigma[2] < 1 && mu[2] - 3*sigma[2] > 0,sigma[2],prev_sigma[2])
  ...
  # Return the final parameters and log-likelihood
